...
def s_scores_hist(file_B1H):
    s_scores = file_B1H. drop(columns=['OLS ID', 'Gene/Range', 'Organism', 'ZF_protein_sequence'], axis=1)
    # np.histogram(s_scores.values)
    plt.hist(s_scores.values, bins=100)
    plt.title("B1H S SCORES distribution")
    plt.xlabel("scores")
    plt.ylabel("scores amount")
    plt.show()
    return


def s_scores_vec_hist(file_B1H):
    s_scores = file_B1H. drop(columns=['OLS ID', 'Gene/Range', 'Organism', 'ZF_protein_sequence'], axis=1)
    s = s_scores.values
    s = s.flatten()
    plt.hist(s, bins=120)
...
def create_all_data_file_for_F_i(folder_address):
    # creating all data file including amino acids, rate and DNA
    mega_file = open(folder_address + 'all_data.txt', 'w')

    for files in os.listdir(folder_address):
        # print(files)
        if len(files) < 15:  # this is not an amino acid file
            continue
        if len(files) == 17:
            current_file = open(folder_address + files, 'r')
            current_file.close()
            continue

        current_file = open(folder_address + files, 'r')
        while True:
            line = current_file.readline().replace('\n', '')  # copy the file txt
            if not line:
                break
            mega_file.write(line + ' ' + files[0:3] + '\n')  # write the file line and the DNA
...
def bind_intensity_histogram(score):
    fig = plt.figure()
    plt.hist(score, bins=40)
    plt.title("F3: log(scores) distribution")
...
def create_file_B1H_suffix(file_B1H):
    """ this function is responsible to divide file_B1H into 8 data frames: each df have: zf, dna, core seq, b1h_s_score
        The separation is according to the suffix of the dna"""
    file_B1H_A1 = pd.concat(
        [file_B1H['ZF_protein_sequence'], file_B1H['core_seq'], file_B1H.filter(regex='A_1$', axis=1)], axis=1)
    file_B1H_C1 = pd.concat(
        [file_B1H['ZF_protein_sequence'], file_B1H['core_seq'], file_B1H.filter(regex='C_1$', axis=1)], axis=1)
    file_B1H_G1 = pd.concat(
        [file_B1H['ZF_protein_sequence'], file_B1H['core_seq'], file_B1H.filter(regex='G_1$', axis=1)], axis=1)
    file_B1H_T1 = pd.concat(
        [file_B1H['ZF_protein_sequence'], file_B1H['core_seq'], file_B1H.filter(regex='T_1$', axis=1)], axis=1)
    file_B1H_A2 = pd.concat(
        [file_B1H['ZF_protein_sequence'], file_B1H['core_seq'], file_B1H.filter(regex='A_2$', axis=1)], axis=1)
    file_B1H_C2 = pd.concat(
        [file_B1H['ZF_protein_sequence'], file_B1H['core_seq'], file_B1H.filter(regex='C_2$', axis=1)], axis=1)
    file_B1H_G2 = pd.concat(
        [file_B1H['ZF_protein_sequence'], file_B1H['core_seq'], file_B1H.filter(regex='G_2$', axis=1)], axis=1)
    file_B1H_T2 = pd.concat(
        [file_B1H['ZF_protein_sequence'], file_B1H['core_seq'], file_B1H.filter(regex='T_2$', axis=1)], axis=1)

    file_all_groups = [file_B1H_A1, file_B1H_C1, file_B1H_G1, file_B1H_T1, file_B1H_A2, file_B1H_C2, file_B1H_G2,
                       file_B1H_T2]
    return file_all_groups
